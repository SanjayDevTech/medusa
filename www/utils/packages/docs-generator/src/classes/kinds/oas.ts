...
class OasKindGenerator extends FunctionKindGenerator {
  ...
  getAssociatedWorkflow(node: FunctionNode): string | undefined {
    ...
    // retrieve workflows imported from `@biryanihouse/core-flows`
    ...
    ;(sourceFile.imports as ts.Node[]).forEach((importNode) => {
      if (
        ...
        importNode.getText() !== `"@biryanihouse/core-flows"`
      ) {
        return
      }
    ...
    // if no imports found from `@biryanihouse/core-flows`, return
