...
This doesn't apply to the design-system packages, including `@biryanihouse/ui`, `@biryanihouse/ui-presets`, and `@biryanihouse/ui-icons`. These packages are versioned independently. However, you don't need to install and manage them separately in your Medusa application, as they are included in the `@biryanihouse/admin-sdk`. If you're using them in a standalone project, such as a storefront or custom admin dashboard, refer to [this section in the Medusa UI documentation](!ui!/installation/standalone-project#updating-ui-packages) for update instructions.
...
Then, to update your Medusa application, bump the version of all `@biryanihouse/*` dependencies in your `package.json`. Then, re-install dependencies:
...
Then, revert the update by changing the version of all `@biryanihouse/*` dependencies in your `package.json` to the previous version and re-installing dependencies:
