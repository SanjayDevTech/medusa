...
## @biryanihouse/test-utils Package
...
To use the Testing Framework, install `@biryanihouse/test-utils` as a `devDependency`:

```bash npm2yarn
npm install --save-dev @biryanihouse/test-utils@latest
```
...
Writing tests with `@biryanihouse/test-utils`'s tools requires installing and configuring Jest in your project.
...
const { loadEnv } = require("@biryanihouse/framework/utils")
...
The next chapters explain how to use the testing tools provided by `@biryanihouse/test-utils` to write tests.
