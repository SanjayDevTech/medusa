---
slug: /references/medusa-workflows/steps/deleteStockLocationsStep
sidebar_label: deleteStockLocationsStep
tags:
  - stock location
  - step
---

import { TypeList, WorkflowDiagram } from "docs-ui"

# deleteStockLocationsStep - Medusa Core Workflows Reference

This documentation provides a reference to the `deleteStockLocationsStep`. It belongs to the `@biryanihouse/medusa/core-flows` package.

This step deletes one or more stock locations.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@biryanihouse/framework/workflows-sdk"
import { deleteStockLocationsStep } from "@biryanihouse/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = deleteStockLocationsStep([
      "{value}"
    ])
  }
)
```
  

## Input

<TypeList types={[{"name":"DeleteStockLocationsStepInput","type":"[DeleteStockLocationsStepInput](../../../../types/core_flows.DeleteStockLocationsStepInput/page.mdx)","optional":false,"defaultValue":"","description":"The IDs of stock locations to delete.","expandable":false,"children":[{"name":"DeleteStockLocationsStepInput","type":"`string`[]","description":"The IDs of stock locations to delete.","optional":false,"defaultValue":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.medusajs.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="deleteStockLocationsStep"/>

## Output

<TypeList types={[{"name":"DeleteEntityInput","type":"[DeleteEntityInput](../../../../../modules_sdk/types/modules_sdk.DeleteEntityInput/page.mdx)","optional":false,"defaultValue":"","description":"The details of a data model's record whose linked records should be deleted. Usually used after the\ndata model's record is deleted.\n\nThe key is the data model's name. Its value is an object that has the ID of the data model's record.","expandable":false,"children":[{"name":"DeleteEntityInput","type":"`object`","description":"The details of a data model's record whose linked records should be deleted. Usually used after the\ndata model's record is deleted.\n\nThe key is the data model's name. Its value is an object that has the ID of the data model's record.","optional":false,"defaultValue":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.medusajs.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="deleteStockLocationsStep"/>
