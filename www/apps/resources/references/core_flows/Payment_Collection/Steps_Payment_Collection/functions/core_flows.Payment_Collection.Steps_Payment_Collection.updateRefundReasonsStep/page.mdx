...
This documentation provides a reference to the `updateRefundReasonsStep`. It belongs to the `@biryanihouse/medusa/core-flows` package.

This step updates one or more refund reasons.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@biryanihouse/framework/workflows-sdk"
import { updateRefundReasonsStep } from "@biryanihouse/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = updateRefundReasonsStep({
      "id": "id_WmtsQhEGjNOQOrgp"
    })
  }
)
```
