...
1. `createProductsWorkflow`: Create the Medusa product that the digital product is associated with its variant. Medusa provides this workflow through the `@biryanihouse/medusa/core-flows` package, which you can use as a step.
...
4. `createRemoteLinkStep`: Create the link between the digital product and the product variant. Medusa provides this step through the `@biryanihouse/medusa/core-flows` package.
...
import { 
  AdminCreateProduct,
} from "@biryanihouse/medusa/api/admin/products/validators"
import { z } from "zod"
import { MediaType } from "../modules/digital-product/types"
...
import { uploadFilesWorkflow } from "@biryanihouse/medusa/core-flows"
...
import { ProductVariantDTO } from "@biryanihouse/framework/types"
...
import { PhotoSolid } from "@biryanihouse/icons"
import { Container, Heading, Table } from "@biryanihouse/ui"
...
import { useState } from "react"
import { Input, Button, Select, toast } from "@biryanihouse/ui"
import { MediaType } from "../../types"
...
```tsx title="src/admin/routes/digital-products/page.tsx"
// other imports...
import { Drawer } from "@biryanihouse/ui"
...
import { ModuleProviderExports } from "@biryanihouse/framework/types"
...
1. `completeCartWorkflow` to create a Medusa order from the cart. Medusa provides this workflow through the `@biryanihouse/medusa/core-flows` package and you can use it as a step.
2. `useQueryGraphStep` to retrieve the order’s items with the digital products associated with the purchased product variants. Medusa provides this step through the `@biryanihouse/medusa/core-flows` package.
3. If the order has digital products, you:
    1. create the digital product order.
    2. link the digital product order with the Medusa order. Medusa provides a `createRemoteLinkStep` in the `@biryanihouse/medusa/core-flows` package that can be used here.
    3. Create a fulfillment for the digital products in the order. Medusa provides a `createOrderFulfillmentWorkflow` in the `@biryanihouse/medusa/core-flows` package that you can use as a step here.
    4. Emit the `digital_product_order.created` custom event to handle it later in a subscriber and send the customer an email. Medusa provides a `emitEventStep` in the `@biryanihouse/medusa/core-flows` that you can use as a step here.
...
```ts
import { OrderDTO, InferTypeOf } from "@biryanihouse/framework/types"
import DigitalProductOrder from "../models/digital-product-order"
