import { Github, PlaySolid } from "@biryanihouse/icons"
...
import CartModule from "@biryanihouse/medusa/cart"
...
import { defineLink } from "@biryanihouse/framework/utils"
import SubscriptionModule from "../modules/subscription"
import CustomerModule from "@biryanihouse/medusa/customer"
...
import { defineLink } from "@biryanihouse/framework/utils"
import SubscriptionModule from "../modules/subscription"
import OrderModule from "@biryanihouse/medusa/order"
...
```ts title="src/modules/subscription/types/index.ts"
import { InferTypeOf } from "@biryanihouse/framework/types"
import Subscription from "../models/subscription"
...
Medusa provides the first and last steps in the `@biryanihouse/medusa/core-flows` package, so you only need to implement the second step.
...
import { 
  ...
  useQueryGraphStep,
} from "@biryanihouse/medusa/core-flows"
import { 
  SubscriptionInterval,
...
1. `completeCartWorkflow` from `@biryanihouse/medusa/core-flows` that completes a cart and creates an order.
2. `useQueryGraphStep` from `@biryanihouse/medusa/core-flows` to retrieve the order's details.
3. `createSubscriptionStep`, which is the step you created previously.
4. `createRemoteLinkStep` from `@biryanihouse/medusa/core-flows`, which accepts links to create. These links are in the `linkDefs` array returned by the previous step.
...
import { StoreCart } from "@biryanihouse/framework/types"
import { Button, clx, Heading, Text } from "@biryanihouse/ui"
import { CheckCircleSolid } from "@biryanihouse/icons"
...
import { ClockSolid } from "@biryanihouse/icons"
import { Container, Heading, Table, Badge } from "@biryanihouse/ui"
...
1. Retrieve the subscription’s linked cart. Medusa provides a `useQueryGraphStep` in the `@biryanihouse/medusa/core-flows` package that can be used as a step.
2. Create a payment collection for the new order. Medusa provides a `createPaymentCollectionsStep` in the `@biryanihouse/medusa/core-flows` package that you can use.
3. Create payment sessions in the payment collection. Medusa provides a `createPaymentSessionsWorkflow` in the `@biryanihouse/medusa/core-flows` package that can be used as a step.
4. Authorize the payment session. Medusa also provides the `authorizePaymentSessionStep` in the `@biryanihouse/medusa/core-flows` package, which can be used.
5. Create the subscription’s new order.
6. Create links between the subscription and the order using the `createRemoteLinkStep` provided in the `@biryanihouse/medusa/core-flows` package.
7. Capture the order’s payment using the `capturePaymentStep` provided by Medusa in the `@biryanihouse/medusa/core-flows` package.
...
import { createOrdersWorkflow } from "@biryanihouse/medusa/core-flows"
...
This creates a `createSubscriptionOrderStep` that uses the `createOrdersWorkflow`, which Medusa provides in the `@biryanihouse/medusa/core-flows` package. The step returns the created order and an array of links to be created.
...
import { createWorkflow, WorkflowResponse } from "@biryanihouse/framework/workflows-sdk"
import { 
  useQueryGraphStep,
