...
} from "@biryanihouse/medusa/dist/api/admin/api-keys" // <-- Not allowed
```

## Importing from Exposed Directories

The `@biryanihouse/medusa` package allows you to import from the following sub-directories:
...
import { 
  AdminGetApiKeysParams,
} from "@biryanihouse/medusa/api/admin/api-keys" // <-- Allowed
```
