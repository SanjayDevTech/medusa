...
<PackageInstall packageName="@biryanihouse/ui-preset" devDependency />
...
`@biryanihouse/ui` is included in the content field of your `tailwind.config.js` file:

```tsx
module.exports = {
  content: [
    // ...
    "./node_modules/@biryanihouse/ui/dist/**/*.{js,jsx,ts,tsx}",
  ],
  // ...
}
```

If you are working within a monorepo, you may need to add the path to the `@biryanihouse/ui` package in your `tailwind.config.js` like so:

```tsx
const path = require("path")

const uiPath = path.resolve(
  require.resolve("@biryanihouse/ui"),
  "../..",
  "\*_/_.{js,jsx,ts,tsx}"
)
...
```tsx
import { Button, Drawer } from "@biryanihouse/ui"
```
