import { getConfigFile } from "@biryanihouse/framework/utils"

export async function configLoaderOverride(
  entryDirectory: string,
  override: { clientUrl: string; debug?: boolean }
) {
  const { configManager } = await import("@biryanihouse/framework/config")
  const { logger } = await import("@biryanihouse/framework")
