...
`yarn add --dev @biryanihouse/medusa-oas-cli`

Install in the global namespace is not yet supported.
~~`npm install -g @biryanihouse/medusa-oas-cli`~~
...
This command will scan the `@biryanihouse/medusa` package in order to extract JSDoc OAS into a json file.
