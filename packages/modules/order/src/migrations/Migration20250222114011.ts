...
export class Migration20250222114011 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table if exists "order" add column if not exists "location_id" text null;`);
    ...
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_order_location_id" ON "order" (location_id) WHERE deleted_at IS NOT NULL;`);
  ...
  override async down(): Promise<void> {
    this.addSql(`drop index if exists "IDX_order_location_id";`);
    this.addSql(`alter table if exists "order" drop column if exists "location_id";`);
