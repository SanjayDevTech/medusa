...
import { ApiRoutesLoader } from "@biryanihouse/framework/http"
import { Tracer } from "@biryanihouse/framework/telemetry"
import type { SpanExporter } from "@opentelemetry/sdk-trace-node"
import type { Instrumentation } from "@opentelemetry/instrumentation"
import { TransactionOrchestrator } from "@biryanihouse/framework/orchestration"
...
export function instrumentHttpLayer() {
  ...
  const HTTPTracer = new Tracer("@biryanihouse/http", "2.0.0")
...
export function instrumentRemoteQuery() {
  const QueryTracer = new Tracer("@biryanihouse/query", "2.0.0")
  const { SpanStatusCode } = require("@opentelemetry/api")
...
export function instrumentWorkflows() {
  const WorkflowsTracer = new Tracer(
    "@biryanihouse/framework/workflows-sdk",
    "2.0.0"
  )
